...
| 指标 | 数值 | 详情 |
|------|------|------|
| **代码行数** | 49,490 行 | [代码结构](docs/architecture/structure.md) |
...
**模块分布**：
- 文件系统 (fs/): 10,974 行 (22.2%)
- 架构相关 (arch/): 9,413 行 (19.0%)
- 单元测试 (tests/): 7,039 行 (14.2%)
- 设备驱动 (drivers/): 5,736 行 (11.6%)
- 内存管理 (mm/): 4,295 行 (8.7%)
- 网络协议栈 (net/): 3,608 行 (7.3%)
- 进程调度 (sched/): 2,248 行 (4.5%)
- 进程管理 (process/): 1,333 行 (2.7%)
- 同步原语 (sync/): 684 行 (1.4%)
...
```
Rux/
├── kernel/                 # 内核源码 (49,490 行)
│   ├── src/
│   │   ├── fs/           # 文件系统 (10,974 行)
│   │   ├── arch/         # RISC-V 架构 (9,413 行)
│   │   ├── drivers/      # 设备驱动 (5,736 行)
│   │   ├── tests/        # 单元测试 (7,039 行)
│   │   ├── net/          # 网络协议栈 (3,608 行)
│   │   ├── mm/           # 内存管理 (4,295 行)
│   │   ├── sched/        # 进程调度 (2,248 行)
│   │   ├── process/      # 进程管理 (1,333 行)
│   │   └── sync/         # 同步原语 (684 行)
│   └── build.rs          # 构建脚本
├── docs/                 # 📚 文档中心
├── test/                 # 测试脚本
