...
## 2026年新增审查发现 (2026-02-10)

### 审查说明
**审查范围**：RISC-V 64位架构全面代码审查
**审查方法**：系统性代码审查 + 与 Linux 内核对比
**审查日期**：2026-02-10
...
#### 28. 页帧分配器无法释放内存 🔴 **严重**
**文件**：`kernel/src/mm/page.rs:158-161`
**问题描述**：
```rust
...
- 所有分配的物理页都无法回收
- 长时间运行会导致内存耗尽
- fork()、execve() 等系统调用会持续泄漏内存

**对比 Linux**：
- Linux 使用伙伴系统（buddy allocator）管理空闲页帧
- `free_pages()` 维护空闲链表
- 支持页面合并和分割
...
struct FrameAllocator {
    free_list: [*mut FreeList; MAX_ORDER],
    // 或使用 buddy allocator
}
```

**优先级**：🔴 严重（内存泄漏，系统无法长时间运行）

---

#### 29. TrapFrame 和 SyscallFrame 字段不匹配 🔴 **严重**
**文件**：
- `kernel/src/arch/riscv64/trap.rs:21-53`
- `kernel/src/arch/riscv64/syscall.rs:89-116`
...
pub struct TrapFrame {
    x1: u64,      // sp
    x5: u64, x6: u64, x7: u64,
    x10: u64,     // a0
    x11: u64,     // a1
    x12: u64,     // a2
    x13: u64,     // a3
    x14: u64, x15: u64, x16: u64, x17: u64, // a4-a7
    x18: u64, x19: u64, ..., x31: u64,
    sstatus: u64, sepc: u64, stval: u64,
}

// syscall.rs 的 SyscallFrame
pub struct SyscallFrame {
    ...
    pub t4: u64, pub t5: u6: u64,
    pub s0: u64, pub s1: u64, ..., pub s11: u64,
    pub ra: u64, pub sp: u64, pub gp: u64, pub tp: u64,
    pub pc: u64, pub status: u64,
}
```

**问题代码**（`trap.rs:294-328`）：
```rust
// 手动字段转换，容易出错
let mut syscall_frame = SyscallFrame {
    a0: (*frame).x10,   // 注意：x10 对应 a0
    a1: (*frame).x11,   // x11 对应 a1
    a2: (*frame).x12,
    // ...
    t0: (*frame).x5,    // x5 对应 t0
    // ...
};
```
...
**对比 Linux**：
- Linux RISC-V 使用统一的 `struct pt_regs`
- 系统调用直接从 pt_regs 读取参数
- 无需转换

**修复方案**：
1. 统一 TrapFrame 和 SyscallFrame 字段布局
2. 或者使用 `#[repr(C)]` 确保布局一致
3. 减少手动转换，使用 `transmute` 或 `union`
...
#### 30. 信号队列设计缺陷 🔴 **严重**
**文件**：`kernel/src/signal.rs:220-244`
**问题描述**：
```rust
pub struct SigQueue {
    pub head: AtomicU64,   // 应该是指针，但这里是 u64？
    pub tail: AtomicU64,
}
```

**问题**：
- AtomicU64 无法存储指针（在 64 位系统上虽然可以，但类型不安全）
- 没有实际的队列实现
- 只检查是否为空，但从未真正入队/出队
...
**对比 Linux**：
- Linux 使用 `struct sigqueue` 链表
- `struct sigpending` 包含信号链表头
- 支持实时信号排队
...
pub struct SigQueueNode {
    info: SigInfo,
    next: Option<*mut SigQueueNode>,
}

pub struct SigQueue {
    head: AtomicPtr<SigQueueNode>,
    tail: AtomicPtr<SigQueueNode>,
}
```
...
#### 32. 架构特定代码在通用模块 🟡 **中等**
**文件**：`kernel/src/process/task.rs:104-148`
**问题描述**：
```rust
#[repr(C)]
#[derive(Debug, Clone)]
...
**问题**：
- 使用 ARM64 寄存器命名 (x19-x28)
- 这是 RISC-V 项目，应该使用 RISC-V 约定
- 或者使用架构无关命名
...
**问题示例**：
```rust
// sys_openat (行 299-307)
let filename = unsafe {
    let mut len = 0;
    let mut ptr = pathname_ptr;
    while len < 256 {  // 只检查长度，不检查地址范围
        let byte = *ptr;
        if byte == 0 { break; }
        len += 1;
        ptr = ptr.add(1);  // 可能越界访问
    }
...
**影响**：
- 用户程序可能故意传递非法地址导致内核崩溃
- 安全漏洞

**对比 Linux**：
- Linux 使用 `access_ok()` 和 `strncpy_from_user()`
- 有严格的用户空间地址验证

**修复方案**：
```rust
pub unsafe fn verify_user_ptr(ptr: u64, size: usize) -> bool {
    const USER_SPACE_END: u64 = 0x0000_ffff_ffff_ffff;
    if ptr > USER_SPACE_END { return false; }
    ptr.checked_add(size as u64)
...
**文件**：多个文件
**问题描述**：
- `trap.rs`: 10+ 处 putchar 输出
- `mm.rs`: 20+ 处 println 输出
- `syscall.rs`: 15+ 处 println 输出
...
```rust
#[cfg(debug_assertions)]
debug_println!("trap: handling exception");

// 或使用日志级别
log::trace!("trap: handling exception");
```
...
#### 35. CpuContext 命名不一致 🟢 **轻微**
**文件**：`kernel/src/process/task.rs`
**问题**：
- `CpuContext` 使用 ARM64 命名 (x19-x28)
- 项目是 RISC-V 架构

**修复方案**：
- 使用架构无关命名 (callee_saved_0, callee_saved_1, ...)
- 或使用条件编译
...
#### 36. 临时调试代码未清理 🟢 **轻微**
**文件**：`kernel/src/process/task.rs:439-446`
```rust
...
#### P0 - 立即修复（影响系统稳定性）
1. **页帧分配器无法释放** - 内存泄漏
2. **TrapFrame/SyscallFrame 不匹配** - 数据损坏风险
3. **信号队列设计缺陷** - 信号丢失
