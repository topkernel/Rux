...
**当前状态**：Phase 2 进行中 🔄 - 中断与进程框架基本完成，MMU 使能问题待解决
...
### 3.1 系统调用接口
- [ ] **系统调用框架** (`syscall/syscall.rs`)
  - [ ] 系统调用表
  - [ ] 参数解析
  - [ ] 返回值处理
- [ ] **系统调用实现**
  - [ ] 进程相关：`fork`、`execve`、`exit`、`wait`、`getpid`
  - [ ] 文件相关：`open`、`close`、`read`、`write`、`lseek`
  - [ ] 内存相关：`brk`、`mmap`、`munmap`
  - [ ] 其他：`ioctl`、`uname`、`gettimeofday`
- [ ] **用户/内核隔离**
  - [ ] 用户栈和内核栈分离
  - [ ] 参数复制（用户空间 → 内核空间）
  - [ ] 结果复制（内核空间 → 用户空间）
